...
{
    public partial class Frm_ImprimirTarjaPT_ASA : Form
    {
        ...
        public Frm_ImprimirTarjaPT_ASA()
        {
            InitializeComponent();
        }

        private void Frm_ImprimirTarjaPT_ASA_Load(object sender, EventArgs e)
        {
            //ALTO RIESGO
            //MAYUSCULA
            PonerOscuroComboBox();
        ...
        public void BuscarRecepcionPT()
        {
            ...
            dt = cn_sst.ListaPTImprimir(dateTimePicker_FecInicio.Value.Date
                , dateTimePicker_FecFin.Value.Date, emp
                , sed, cult);
        ...
        public void ImprimirReporteDetalleAtencionASA()
        {
            ...
            foreach (DataGridViewRow row in dataGridView2.Rows)
            {
                if (Convert.ToBoolean(row.Cells[0].Value) == true)
                {
                    ...
                    objDsRepAcopio.Tables[2].Rows.Add
                            (new object[]{
                        row.Cells[2].Value.ToString().ToUpper(), //PALLET
                        row.Cells[4].Value.ToString().ToUpper(), //STYLE
                        row.Cells[5].Value.ToString().ToUpper(), //SIZE
                        row.Cells[6].Value.ToString(), //CATEGORIA
                        row.Cells[7].Value.ToString(), //CANTIDAD
                        row.Cells[8].Value.ToString(), //ORDEN
                        empresa, //EMP
                        row.Cells[12].Value.ToString()

                            });

                    objcnsst.ActualizarPTImprimir(row.Cells[2].Value.ToString()
                , Convert.ToDateTime(row.Cells[9].Value.ToString()));
            ...
            Acopio.CrystalReportPrtPal objCryRep = new Acopio.CrystalReportPrtPal();
            //string dir = @"~\CryRep_VentasUsuario.rpt";
            objCryRep.Load(@"~\CrystalReportPrtPal.rpt");
