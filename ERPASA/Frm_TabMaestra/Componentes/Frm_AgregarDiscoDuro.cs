...
{
    public partial class Frm_AgregarDiscoDuro : Form
    {
        ...
        private void Frm_AgregarDiscoDuro_Load(object sender, EventArgs e)
        {
            if (textBox_CodDD.Text == "")
            {
                CargarPanelgeneral();
        ...
        {
            textBox_CodDD.Enabled = false;
            textBox_DesDD.Enabled = false;
            textbox_ObsDD.Enabled = false;
        ...
        {
            textBox_DesDD.Enabled = true;
            textbox_ObsDD.Enabled = true;
        ...
        public void CargarPanelgeneral()
        {
            ...
            cn_logistica.BuscUltimoCodDiscoDuroTIMas();
            if (ce_logistica.codigo.ToString() == "")
            {
                textBox_CodDD.Text = "DD01";
            }
            else
            {
                textBox_CodDD.Text = "DD" + ce_logistica.codigo.ToString("D2");
            }
            //LABEL            
            comboBox_Est.DropDownStyle = ComboBoxStyle.DropDownList;
            textBox_DesDD.CharacterCasing = CharacterCasing.Upper;
            textbox_ObsDD.CharacterCasing = CharacterCasing.Upper;
            //
            //CARGAR CONDICION DEL PRODUCTO
            comboBox_Est.DataSource = cn_regproducto.ListCond();
            comboBox_Est.DisplayMember = "DesEstado"; //NOMBRE DE LA TABLA O NOMBRE DE COLUMNA ID
        ...
        private void Guardar()
        {
            if (textBox_DesDD.Text != "")
            {
                ...
                cn_logistica.BuscExisteCodDiscoDuroTI(textBox_CodDD.Text);
                if (textBox_CodDD.Text == ce_logistica.CodTiExist)
                {
                    ...
                    {
                        objdatlogisticaneg.ActuaDiscoDuroTI(
                        textBox_CodDD.Text,
                        textBox_DesDD.Text,
                        textbox_ObsDD.Text,
                ...
                else if (textBox_CodDD.Text != ce_logistica.CodTiExist)
                {
                    ...
                    if (dialogResult == DialogResult.Yes)
                    {
                        objdatlogisticaneg.GuardDiscoDuroTI(
                        textBox_CodDD.Text,
                        textBox_DesDD.Text,
                        textbox_ObsDD.Text,
                        comboBox_Est.SelectedValue.ToString(),
        ...
        private void Eliminar()
        {
            ...
            if (textBox_CodDD.Text == ce_logistica.CodTiExist)
            {
                ...
                if (dialogResult == DialogResult.Yes)
                {
                    objdatlogisticaneg.EliminDiscoDuroTI(textBox_CodDD.Text);
