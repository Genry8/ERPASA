...
{
    public partial class Frm_AgregarMemoria : Form
    {
        ...
        private void Frm_AgregarMemoria_Load(object sender, EventArgs e)
        {
            if (textBox_CodMemoria.Text == "")
            {
                CargarPanelgeneral();
                ...
                comboBox_Memoria.DataSource = cn_regproducto.ListCond();
                comboBox_Memoria.DisplayMember = "DesEstado"; //NOMBRE DE LA TABLA O NOMBRE DE COLUMNA ID
                comboBox_Memoria.ValueMember = "id"; // NOMBRE DE LA COLUMNA o PARA ORDENAR
            }
            BloquearTextboxCombobox();
        ...
        {
            textBox_DesMemoria.Text = "";
            textbox_ObsMemoria.Text = "";
        }

        public void BloquearTextboxCombobox()
        {
            textBox_CodMemoria.Enabled = false;
            textBox_DesMemoria.Enabled = false;
            textbox_ObsMemoria.Enabled = false;
            comboBox_Memoria.Enabled = false;
            comboBox_Memoria.DropDownStyle = ComboBoxStyle.DropDownList;
        }

        public void DesbloquearTextboxCombobox()
        {
            textBox_DesMemoria.Enabled = true;
            textbox_ObsMemoria.Enabled = true;
            comboBox_Memoria.Enabled = true;
            comboBox_Memoria.DropDownStyle = ComboBoxStyle.DropDownList;
        }
        ...
        public void CargarPanelgeneral()
        {
            ...
            cn_logistica.BuscUltimoCodMemoriaMas();
            if (ce_logistica.codigo.ToString() == "")
            {
                textBox_CodMemoria.Text = "CM01";
            }
            else
            {
                textBox_CodMemoria.Text = "CM" + ce_logistica.codigo.ToString("D2");
            }
            //LABEL            
            comboBox_Memoria.DropDownStyle = ComboBoxStyle.DropDownList;
            textBox_DesMemoria.CharacterCasing = CharacterCasing.Upper;
            textbox_ObsMemoria.CharacterCasing = CharacterCasing.Upper;
            //
            //CARGAR CONDICION DEL PRODUCTO
            comboBox_Memoria.DataSource = cn_regproducto.ListCond();
            comboBox_Memoria.DisplayMember = "DesEstado"; //NOMBRE DE LA TABLA O NOMBRE DE COLUMNA ID
        ...
        private void Guardar()
        {
            if (textBox_DesMemoria.Text != "")
            {
                ...
                cn_logistica.BuscExisteCodMemoria(textBox_CodMemoria.Text);
                if (textBox_CodMemoria.Text == ce_logistica.CodTiExist)
                {
                    ...
                    {
                        objdatlogisticaneg.ActuaMemoria(
                        textBox_CodMemoria.Text,
                        textBox_DesMemoria.Text,
                        textbox_ObsMemoria.Text,
                        comboBox_Memoria.SelectedValue.ToString(),
                ...
                else if (textBox_CodMemoria.Text != ce_logistica.CodTiExist)
                {
                    ...
                    if (dialogResult == DialogResult.Yes)
                    {
                        objdatlogisticaneg.GuardMemoria(
                        textBox_CodMemoria.Text,
                        textBox_DesMemoria.Text,
                        textbox_ObsMemoria.Text,
                        comboBox_Memoria.SelectedValue.ToString(),
        ...
        private void Eliminar()
        {
            ...
            if (textBox_CodMemoria.Text == ce_logistica.CodTiExist)
            {
                ...
                if (dialogResult == DialogResult.Yes)
                {
                    objdatlogisticaneg.EliminMemoria(textBox_CodMemoria.Text);
