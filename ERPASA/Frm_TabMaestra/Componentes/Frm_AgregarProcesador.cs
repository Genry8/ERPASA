...
{
    public partial class Frm_AgregarProcesador : Form
    {
        ...
        private void Frm_AgregarProcesador_Load(object sender, EventArgs e)
        {
            if (textBox_CodProcesador.Text == "")
            {
                CargarPanelgeneral();
                ...
                comboBox_Est.DataSource = cn_regproducto.ListCond();
                comboBox_Est.DisplayMember = "DesEstado"; //NOMBRE DE LA TABLA O NOMBRE DE COLUMNA ID
                comboBox_Est.ValueMember = "id"; // NOMBRE DE LA COLUMNA o PARA ORDENAR
            }
            BloquearTextboxCombobox();
        ...
        {
            textBox_DesProcesador.Text = "";
            textbox_ObsProcesador.Text = "";
        }

        public void BloquearTextboxCombobox()
        {
            textBox_CodProcesador.Enabled = false;
            textBox_DesProcesador.Enabled = false;
            textbox_ObsProcesador.Enabled = false;
            comboBox_Est.Enabled = false;
            comboBox_Est.DropDownStyle = ComboBoxStyle.DropDownList;
        }

        public void DesbloquearTextboxCombobox()
        {
            textBox_DesProcesador.Enabled = true;
            textbox_ObsProcesador.Enabled = true;
            comboBox_Est.Enabled = true;
            comboBox_Est.DropDownStyle = ComboBoxStyle.DropDownList;
        }
        ...
        public void CargarPanelgeneral()
        {
            ...
            cn_logistica.BuscUltimoCodProcesadorTIMas();
            if (ce_logistica.codigo.ToString() == "")
            {
                textBox_CodProcesador.Text = "PR01";
            }
            else
            {
                textBox_CodProcesador.Text = "PR" + ce_logistica.codigo.ToString("D2");
            }
            //LABEL            
            comboBox_Est.DropDownStyle = ComboBoxStyle.DropDownList;
            textBox_DesProcesador.CharacterCasing = CharacterCasing.Upper;
            textbox_ObsProcesador.CharacterCasing = CharacterCasing.Upper;
            //
            //CARGAR CONDICION DEL PRODUCTO
            comboBox_Est.DataSource = cn_regproducto.ListCond();
            comboBox_Est.DisplayMember = "DesEstado"; //NOMBRE DE LA TABLA O NOMBRE DE COLUMNA ID
        ...
        private void Guardar()
        {
            if (textBox_DesProcesador.Text != "")
            {
                ...
                cn_logistica.BuscExisteCodProcesadorTI(textBox_CodProcesador.Text);
                if (textBox_CodProcesador.Text == ce_logistica.CodTiExist)
                {
                    string MessageBoxTitle = "Actualizar";
                    string MessageBoxContent = "¿Desea actualizar Estado?";

                    DialogResult dialogResult = MessageBox.Show(MessageBoxContent, MessageBoxTitle, MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
                    if (dialogResult == DialogResult.Yes)
                    {
                        objdatlogisticaneg.ActuaProcesadorTI(
                        textBox_CodProcesador.Text,
                        textBox_DesProcesador.Text,
                        textbox_ObsProcesador.Text,
                        comboBox_Est.SelectedValue.ToString(),
                        ...
                        Msbox.Frm_Notificaciones ng = new Msbox.Frm_Notificaciones("ESTADO ACTUALIZADO", Color.FromArgb(13, 93, 140), 1);
                        ng.ShowDialog();
                        this.Close();
                ...
                else if (textBox_CodProcesador.Text != ce_logistica.CodTiExist)
                {
                    string MessageBoxTitle = "Guardar";
                    string MessageBoxContent = "¿Desea Guardar Estado?";

                    DialogResult dialogResult = MessageBox.Show(MessageBoxContent, MessageBoxTitle, MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
                    if (dialogResult == DialogResult.Yes)
                    {
                        objdatlogisticaneg.GuardProcesadorTI(
                        textBox_CodProcesador.Text,
                        textBox_DesProcesador.Text,
                        textbox_ObsProcesador.Text,
                        comboBox_Est.SelectedValue.ToString(),
                        ...
                        Msbox.Frm_Notificaciones ng = new Msbox.Frm_Notificaciones("ESTADO AGREGADO", Color.FromArgb(13, 93, 140), 1);
                        ng.ShowDialog();
                        this.Close();
                    }
                }
        ...
        private void Eliminar()
        {
            ...
            if (textBox_CodProcesador.Text == ce_logistica.CodTiExist)
            {
                ...
                string MessageBoxContent = "¿Desea eliminar Estado?";

                DialogResult dialogResult = MessageBox.Show(MessageBoxContent, MessageBoxTitle, MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
                if (dialogResult == DialogResult.Yes)
                {
                    objdatlogisticaneg.EliminProcesadorTI(textBox_CodProcesador.Text);
                    Msbox.Frm_Notificaciones ng = new Msbox.Frm_Notificaciones("ESTADO ELIMINADO", Color.OrangeRed, 3);
                    ng.ShowDialog();
                    Close();
                    CargarPanelgeneral();
                    Limpiartextbox();
            ...
            {
                Msbox.Frm_Notificaciones ng = new Msbox.Frm_Notificaciones("ESTADO NO ENCONTRADO", Color.OrangeRed, 3);
                ng.ShowDialog();
            }
        }
