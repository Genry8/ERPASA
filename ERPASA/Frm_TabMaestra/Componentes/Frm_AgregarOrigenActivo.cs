...
{
    public partial class Frm_AgregarOrigenActivo : Form
    {
        ...
        public Frm_AgregarOrigenActivo()
        {
            InitializeComponent();
        }
        ...
        private void Frm_AgregarOrigenActivo_Load(object sender, EventArgs e)
        {
            if (textBox_CodOrigenActivo.Text == "")
            {
                CargarPanelgeneral();
        ...
        {
            textBox_DesOrigen.Text = "";
            textbox_ObsOrigen.Text = "";
        }

        public void BloquearTextboxCombobox()
        {
            textBox_CodOrigenActivo.Enabled = false;
            textBox_DesOrigen.Enabled = false;
            textbox_ObsOrigen.Enabled = false;
        ...
        {
            textBox_DesOrigen.Enabled = true;
            textbox_ObsOrigen.Enabled = true;
        ...
        public void CargarPanelgeneral()
        {
            ...
            cn_logistica.BuscUltimoCodOrigenActTIMas();
            if (ce_logistica.codigo.ToString() == "")
            {
                textBox_CodOrigenActivo.Text = "OR01";
            }
            else
            {
                textBox_CodOrigenActivo.Text = "OR" + ce_logistica.codigo.ToString("D2");
            }
            //LABEL            
            comboBox_Estado.DropDownStyle = ComboBoxStyle.DropDownList;
            textBox_DesOrigen.CharacterCasing = CharacterCasing.Upper;
            textbox_ObsOrigen.CharacterCasing = CharacterCasing.Upper;
            //
            //CARGAR CONDICION DEL PRODUCTO
            comboBox_Estado.DataSource = cn_regproducto.ListCond();
            comboBox_Estado.DisplayMember = "DesEstado"; //NOMBRE DE LA TABLA O NOMBRE DE COLUMNA ID
        ...
        private void Guardar()
        {
            if (textBox_DesOrigen.Text != "")
            {
                ...
                cn_logistica.BuscExisteCodOrigenActTI(textBox_CodOrigenActivo.Text);
                if (textBox_CodOrigenActivo.Text == ce_logistica.CodTiExist)
                {
                    ...
                    {
                        objdatlogisticaneg.ActuaOrigenActTI(
                        textBox_CodOrigenActivo.Text,
                        textBox_DesOrigen.Text,
                        textbox_ObsOrigen.Text,
                ...
                else if (textBox_CodOrigenActivo.Text != ce_logistica.CodTiExist)
                {
                    ...
                    if (dialogResult == DialogResult.Yes)
                    {
                        objdatlogisticaneg.GuardOrigenActTI(
                        textBox_CodOrigenActivo.Text,
                        textBox_DesOrigen.Text,
                        textbox_ObsOrigen.Text,
                        comboBox_Estado.SelectedValue.ToString(),
        ...
        private void Eliminar()
        {
            ...
            if (textBox_CodOrigenActivo.Text == ce_logistica.CodTiExist)
            {
                ...
                if (dialogResult == DialogResult.Yes)
                {
                    objdatlogisticaneg.EliminOrigenActTI(textBox_CodOrigenActivo.Text);
