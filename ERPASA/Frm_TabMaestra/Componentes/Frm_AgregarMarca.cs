...
namespace ERPASA.Frm_TabMaestra
{
    public partial class Frm_AgregarMarca : Form
    {
        ...
        {
            textBox_DesMarca.Text = "";
            textbox_ObsMarca.Text = "";
        }

        public void BloquearTextboxCombobox()
        {
            textBox_CodMarca.Enabled = false;
            textBox_DesMarca.Enabled = false;
            textbox_ObsMarca.Enabled = false;
        ...
        {
            textBox_DesMarca.Enabled = true;
            textbox_ObsMarca.Enabled = true;
        ...
        public void CargarPanelgeneral()
        {
            ...
            cn_logistica.BuscUltimoCodMarcaTIMas();
            if (ce_logistica.codigo.ToString() == "")
            {
                textBox_CodMarca.Text = "M001";
            }
            else
            {
                textBox_CodMarca.Text = "M" + ce_logistica.codigo.ToString("D3");
            }
            //LABEL            
            comboBox_Estado.DropDownStyle = ComboBoxStyle.DropDownList;
            textBox_DesMarca.CharacterCasing = CharacterCasing.Upper;
            textbox_ObsMarca.CharacterCasing = CharacterCasing.Upper;
            //
            //CARGAR CONDICION DEL PRODUCTO
            comboBox_Estado.DataSource = cn_regproducto.ListCond();
            comboBox_Estado.DisplayMember = "DesEstado"; //NOMBRE DE LA TABLA O NOMBRE DE COLUMNA ID
        ...
        private void Guardar()
        {
            if (textBox_DesMarca.Text != "")
            {
                ...
                cn_logistica.BuscExisteCodMarcaTI(textBox_CodMarca.Text);
                if (textBox_CodMarca.Text == ce_logistica.CodTiExist)
                {
                    ...
                    {
                        objdatlogisticaneg.ActuaMarcaTI(
                        textBox_CodMarca.Text,
                        textBox_DesMarca.Text,
                        textbox_ObsMarca.Text,
                ...
                else if (textBox_CodMarca.Text != ce_logistica.CodTiExist)
                {
                    ...
                    if (dialogResult == DialogResult.Yes)
                    {
                        objdatlogisticaneg.GuardMarcaTI(
                        textBox_CodMarca.Text,
                        textBox_DesMarca.Text,
                        textbox_ObsMarca.Text,
                        comboBox_Estado.SelectedValue.ToString(),
        ...
        private void Eliminar()
        {
            ...
            if (textBox_CodMarca.Text == ce_logistica.CodTiExist)
            {
                ...
                if (dialogResult == DialogResult.Yes)
                {
                    objdatlogisticaneg.EliminMarcaTI(textBox_CodMarca.Text);
