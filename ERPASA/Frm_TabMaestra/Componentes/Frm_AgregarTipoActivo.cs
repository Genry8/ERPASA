...
{
    public partial class Frm_AgregarTipoActivo : Form
    {
        ...
        private void Frm_AgregarTipoActivo_Load(object sender, EventArgs e)
        {
            if (textBox_CodTipAct.Text == "")
            {
                CargarPanelgeneral();
        ...
        {
            textBox_DesTipo.Text = "";
            textbox_ObsTipAct.Text = "";
        }

        public void BloquearTextboxCombobox()
        {
            textBox_CodTipAct.Enabled = false;
            textBox_DesTipo.Enabled = false;
            textbox_ObsTipAct.Enabled = false;
        ...
        {
            textBox_DesTipo.Enabled = true;
            textbox_ObsTipAct.Enabled = true;
        ...
        public void CargarPanelgeneral()
        {
            ...
            cn_logistica.BuscUltimoCodTipoActivoTIMas();
            if (ce_logistica.codigo.ToString() == "")
            {
                textBox_CodTipAct.Text = "TA01";
            }
            else
            {
                textBox_CodTipAct.Text = "TA" + ce_logistica.codigo.ToString("D2");
            }
            //LABEL            
            comboBox_EstSede.DropDownStyle = ComboBoxStyle.DropDownList;
            textBox_DesTipo.CharacterCasing = CharacterCasing.Upper;
            textbox_ObsTipAct.CharacterCasing = CharacterCasing.Upper;
            //
            //CARGAR CONDICION DEL PRODUCTO
            comboBox_EstSede.DataSource = cn_regproducto.ListCond();
            comboBox_EstSede.DisplayMember = "DesEstado"; //NOMBRE DE LA TABLA O NOMBRE DE COLUMNA ID
        ...
        private void GuardarSede()
        {
            if (textBox_DesTipo.Text != "")
            {
                ...
                cn_logistica.BuscExisteCodTipoActivoTI(textBox_CodTipAct.Text);
                if (textBox_CodTipAct.Text == ce_logistica.CodTiExist)
                {
                    ...
                    {
                        objdatlogisticaneg.ActuaTipoActivoTI(
                        textBox_CodTipAct.Text,
                        textBox_DesTipo.Text,
                        textbox_ObsTipAct.Text,
                ...
                else if (textBox_CodTipAct.Text != ce_logistica.CodTiExist)
                {
                    ...
                    if (dialogResult == DialogResult.Yes)
                    {
                        objdatlogisticaneg.GuardTipoActivoTI(
                        textBox_CodTipAct.Text,
                        textBox_DesTipo.Text,
                        textbox_ObsTipAct.Text,
                        comboBox_EstSede.SelectedValue.ToString(),
        ...
        private void Eliminar()
        {
            ...
            if (textBox_CodTipAct.Text == ce_logistica.CodTiExist)
            {
                ...
                if (dialogResult == DialogResult.Yes)
                {
                    objdatlogisticaneg.EliminTipoActivoTI(textBox_CodTipAct.Text);
