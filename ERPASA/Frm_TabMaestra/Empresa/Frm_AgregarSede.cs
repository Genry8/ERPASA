...
{
    public partial class Frm_AgregarSede : Form
    {
        ...
        private void Frm_AgregarSede_Load(object sender, EventArgs e)
        {
            if (textBox_CodSede.Text == "")
            {
                CargarPanelgeneral();
                ...
                comboBox_EstSede.DataSource = cn_regproducto.ListCond();
                comboBox_EstSede.DisplayMember = "DesEstado"; //NOMBRE DE LA TABLA O NOMBRE DE COLUMNA ID
                comboBox_EstSede.ValueMember = "id"; // NOMBRE DE LA COLUMNA o PARA ORDENAR
            }
            BloquearTextboxCombobox();
        ...
        {
            textBox_DesSede.Text = "";
            textbox_ObsSede.Text = "";
        }

        public void BloquearTextboxCombobox()
        {
            textBox_CodSede.Enabled = false;
            textBox_DesSede.Enabled = false;
            textbox_ObsSede.Enabled = false;
            comboBox_Empresa.Enabled = false;
            comboBox_EstSede.Enabled = false;
            comboBox_Empresa.DropDownStyle = ComboBoxStyle.DropDownList;
            comboBox_EstSede.DropDownStyle = ComboBoxStyle.DropDownList;
        }

        public void DesbloquearTextboxCombobox()
        {
            textBox_DesSede.Enabled = true;
            textbox_ObsSede.Enabled = true;
            comboBox_Empresa.Enabled = true;
            comboBox_EstSede.Enabled = true;
            comboBox_Empresa.DropDownStyle = ComboBoxStyle.DropDownList;
            comboBox_EstSede.DropDownStyle = ComboBoxStyle.DropDownList;
        }
        ...
        public void CargarPanelgeneral()
        {
            ...
            {
                textBox_CodSede.Text = "CS001";
            }
            else
            {
                textBox_CodSede.Text = "CS" + ce_logistica.codigo.ToString("D3");
            }
            //LABEL            
            comboBox_Empresa.DropDownStyle = ComboBoxStyle.DropDownList;
            comboBox_EstSede.DropDownStyle = ComboBoxStyle.DropDownList;
            textBox_DesSede.CharacterCasing = CharacterCasing.Upper;
            textbox_ObsSede.CharacterCasing = CharacterCasing.Upper;
        ...
        private void GuardarSede()
        {
            if (textBox_DesSede.Text != "")
            {
                ...
                cn_logistica.BuscExisteCodSede(textBox_CodSede.Text);
                if (textBox_CodSede.Text == ce_logistica.CodTiExist)
                {
                    string MessageBoxTitle = "Actualizar";
                    string MessageBoxContent = "¿Desea actualizar Sede?";

                    DialogResult dialogResult = MessageBox.Show(MessageBoxContent, MessageBoxTitle, MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
                    if (dialogResult == DialogResult.Yes)
                    {
                        objdatlogisticaneg.ActuaSede(
                        textBox_CodSede.Text,
                        textBox_DesSede.Text,
                        comboBox_Empresa.SelectedValue.ToString(),
                        textbox_ObsSede.Text,
                        comboBox_EstSede.SelectedValue.ToString(),
                        ...
                        Msbox.Frm_Notificaciones ng = new Msbox.Frm_Notificaciones("SEDE ACTUALIZADO", Color.FromArgb(13, 93, 140), 1);
                        ng.ShowDialog();
                        this.Close();
                ...
                else if (textBox_CodSede.Text != ce_logistica.CodTiExist)
                {
                    string MessageBoxTitle = "Guardar";
                    string MessageBoxContent = "¿Desea Guardar Sede?";

                    DialogResult dialogResult = MessageBox.Show(MessageBoxContent, MessageBoxTitle, MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
                    if (dialogResult == DialogResult.Yes)
                    {
                        objdatlogisticaneg.GuardSede(
                        textBox_CodSede.Text,
                        textBox_DesSede.Text,
                        comboBox_Empresa.SelectedValue.ToString(),
                        textbox_ObsSede.Text,
                        ...
                        Msbox.Frm_Notificaciones ng = new Msbox.Frm_Notificaciones("SEDE AGREGADO", Color.FromArgb(13, 93, 140), 1);
                        ng.ShowDialog();
                        this.Close();
                    }
                }
        ...
        private void EliminarSede()
        {
            ...
            if (textBox_CodSede.Text == ce_logistica.CodTiExist)
            {
                ...
                string MessageBoxContent = "¿Desea eliminar Sede?";

                DialogResult dialogResult = MessageBox.Show(MessageBoxContent, MessageBoxTitle, MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
                if (dialogResult == DialogResult.Yes)
                {
                    objdatlogisticaneg.EliminSede(textBox_CodSede.Text);
                    Msbox.Frm_Notificaciones ng = new Msbox.Frm_Notificaciones("SEDE ELIMINADO", Color.OrangeRed, 3);
                    ng.ShowDialog();
                    Close();
                    CargarPanelgeneral();
                    Limpiartextbox();
            ...
            {
                Msbox.Frm_Notificaciones ng = new Msbox.Frm_Notificaciones("SEDE NO ENCONTRADO", Color.OrangeRed, 3);
                ng.ShowDialog();
            }
        }
