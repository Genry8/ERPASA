...
{
    public partial class Frm_AgregarGerencia : Form
    {
        ...
        private void Frm_AgregarGerencia_Load(object sender, EventArgs e)
        {
            if (textBox_CodGerencia.Text == "")
            {
                CargarPanelgeneral();
                ...
                comboBox_EstGerencia.DataSource = cn_regproducto.ListCond();
                comboBox_EstGerencia.DisplayMember = "DesEstado"; //NOMBRE DE LA TABLA O NOMBRE DE COLUMNA ID
                comboBox_EstGerencia.ValueMember = "id"; // NOMBRE DE LA COLUMNA o PARA ORDENAR
            }
            BloquearTextboxCombobox();
        ...
        {
            textBox_DesGerencia.Text = "";
            textbox_ObsGerencia.Text = "";
        }

        public void BloquearTextboxCombobox()
        {
            textBox_CodGerencia.Enabled = false;
            textBox_DesGerencia.Enabled = false;
            textbox_ObsGerencia.Enabled = false;
            comboBox_EstGerencia.Enabled = false;
            comboBox_EstGerencia.DropDownStyle = ComboBoxStyle.DropDownList;
        }

        public void DesbloquearTextboxCombobox()
        {
            textBox_DesGerencia.Enabled = true;
            textbox_ObsGerencia.Enabled = true;
            comboBox_EstGerencia.Enabled = true;
            comboBox_EstGerencia.DropDownStyle = ComboBoxStyle.DropDownList;
        }
        ...
        public void CargarPanelgeneral()
        {
            ...
            cn_logistica.BuscUltimoCodGerenciaMas();
            if (ce_logistica.codigo.ToString() == "")
            {
                textBox_CodGerencia.Text = "CG001";
            }
            else
            {
                textBox_CodGerencia.Text = "CG" + ce_logistica.codigo.ToString("D3");
            }
            //LABEL            
            comboBox_EstGerencia.DropDownStyle = ComboBoxStyle.DropDownList;
            textBox_DesGerencia.CharacterCasing = CharacterCasing.Upper;
            textbox_ObsGerencia.CharacterCasing = CharacterCasing.Upper;
            //
            //CARGAR CONDICION DEL PRODUCTO
            comboBox_EstGerencia.DataSource = cn_regproducto.ListCond();
            comboBox_EstGerencia.DisplayMember = "DesEstado"; //NOMBRE DE LA TABLA O NOMBRE DE COLUMNA ID
        ...
        private void GuardarGerencia()
        {
            if (textBox_DesGerencia.Text != "")
            {

                cn_logistica.BuscExisteCodGerencua(textBox_CodGerencia.Text);
                if (textBox_CodGerencia.Text == ce_logistica.CodTiExist)
                {
                    string MessageBoxTitle = "Actualizar";
                    string MessageBoxContent = "¿Desea actualizar Gerencia?";

                    DialogResult dialogResult = MessageBox.Show(MessageBoxContent, MessageBoxTitle, MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
                    if (dialogResult == DialogResult.Yes)
                    {
                        objdatlogisticaneg.ActuaGerencia(
                        textBox_CodGerencia.Text,
                        textBox_DesGerencia.Text,
                        textbox_ObsGerencia.Text,
                        comboBox_EstGerencia.SelectedValue.ToString(),
                        ...
                        Msbox.Frm_Notificaciones ng = new Msbox.Frm_Notificaciones("GERENCIA ACTUALIZADO", Color.FromArgb(13, 93, 140), 1);
                        ng.ShowDialog();
                        this.Close();
                ...
                else if (textBox_CodGerencia.Text != ce_logistica.CodTiExist)
                {
                    string MessageBoxTitle = "Guardar";
                    string MessageBoxContent = "¿Desea Guardar Gerencia?";

                    DialogResult dialogResult = MessageBox.Show(MessageBoxContent, MessageBoxTitle, MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
                    if (dialogResult == DialogResult.Yes)
                    {
                        objdatlogisticaneg.GuardGerencia(
                        textBox_CodGerencia.Text,
                        textBox_DesGerencia.Text,
                        textbox_ObsGerencia.Text,
                        comboBox_EstGerencia.SelectedValue.ToString(),
                        ...
                        Msbox.Frm_Notificaciones ng = new Msbox.Frm_Notificaciones("GERENCIA AGREGADO", Color.FromArgb(13, 93, 140), 1);
                        ng.ShowDialog();
                        this.Close();
                    }
                }
        ...
        {
            cn_logistica.BuscExisteCodSede(textBox_CodGerencia.Text);
            if (textBox_CodGerencia.Text == ce_logistica.CodTiExist)
            {
                string MessageBoxTitle = "Eliminar";
                string MessageBoxContent = "¿Desea eliminar Gerencia?";

                DialogResult dialogResult = MessageBox.Show(MessageBoxContent, MessageBoxTitle, MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
                if (dialogResult == DialogResult.Yes)
                {
                    objdatlogisticaneg.EliminGerencia(textBox_CodGerencia.Text);
                    Msbox.Frm_Notificaciones ng = new Msbox.Frm_Notificaciones("GERENCIA ELIMINADO", Color.OrangeRed, 3);
                    ng.ShowDialog();
                    Close();
                    CargarPanelgeneral();
                    Limpiartextbox();
            ...
            {
                Msbox.Frm_Notificaciones ng = new Msbox.Frm_Notificaciones("GERENCIA NO ENCONTRADO", Color.OrangeRed, 3);
                ng.ShowDialog();
            }
        }
