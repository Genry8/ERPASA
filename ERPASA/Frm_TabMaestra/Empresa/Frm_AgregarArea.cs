...
{
    public partial class Frm_AgregarArea : Form
    {
        ...
        private void Frm_AgregarArea_Load(object sender, EventArgs e)
        {
            if (textBox_CodArea.Text == "")
            {
                CargarPanelgeneral();
                ...
                //CARGAR GERENCIA
                comboBox_Gerencia.DataSource = cn_logistica.ListaGerencia();
                comboBox_Gerencia.DisplayMember = "DesGer"; //NOMBRE DE LA TABLA O NOMBRE DE COLUMNA ID
                comboBox_Gerencia.ValueMember = "id"; // NOMBRE DE LA COLUMNA o PARA ORDENAR
                //CARGAR CONDICION DEL PRODUCTO
                comboBox_EstArea.DataSource = cn_regproducto.ListCond();
                comboBox_EstArea.DisplayMember = "DesEstado"; //NOMBRE DE LA TABLA O NOMBRE DE COLUMNA ID
                comboBox_EstArea.ValueMember = "id"; // NOMBRE DE LA COLUMNA o PARA ORDENAR
            }
            BloquearTextboxCombobox();
        ...
        {
            textBox_DesArea.Text = "";
            textbox_ObsArea.Text = "";
        }

        public void BloquearTextboxCombobox()
        {
            textBox_CodArea.Enabled = false;
            textBox_DesArea.Enabled = false;
            textbox_ObsArea.Enabled = false;
            comboBox_EstArea.Enabled = false;
            comboBox_Gerencia.DropDownStyle = ComboBoxStyle.DropDownList;
            comboBox_EstArea.DropDownStyle = ComboBoxStyle.DropDownList;
        }

        public void DesbloquearTextboxCombobox()
        {
            textBox_DesArea.Enabled = true;
            textbox_ObsArea.Enabled = true;
            comboBox_EstArea.Enabled = true;
            comboBox_Gerencia.DropDownStyle = ComboBoxStyle.DropDownList;
            comboBox_EstArea.DropDownStyle = ComboBoxStyle.DropDownList;
        }
        ...
        public void CargarPanelgeneral()
        {
            ...
            cn_logistica.BuscUltimoCodAreaMas();
            if (ce_logistica.codigo.ToString() == "")
            {
                textBox_CodArea.Text = "CA001";
            }
            else
            {
                textBox_CodArea.Text = "CA" + ce_logistica.codigo.ToString("D3");
            }
            //LABEL            
            comboBox_Gerencia.DropDownStyle = ComboBoxStyle.DropDownList;
            comboBox_EstArea.DropDownStyle = ComboBoxStyle.DropDownList;
            textBox_DesArea.CharacterCasing = CharacterCasing.Upper;
            textbox_ObsArea.CharacterCasing = CharacterCasing.Upper;
            //
            //CARGAR ESTADO
            comboBox_EstArea.DataSource = cn_regproducto.ListCond();
            comboBox_EstArea.DisplayMember = "DesEstado"; //NOMBRE DE LA TABLA O NOMBRE DE COLUMNA ID
        ...
        private void GuardarArea()
        {
            if (textBox_DesArea.Text != "")
            {
                ...
                cn_logistica.BuscExisteCodArea(textBox_CodArea.Text);
                if (textBox_CodArea.Text == ce_logistica.CodTiExist)
                {
                    string MessageBoxTitle = "Actualizar";
                    string MessageBoxContent = "¿Desea actualizar Area?";

                    DialogResult dialogResult = MessageBox.Show(MessageBoxContent, MessageBoxTitle, MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
                    if (dialogResult == DialogResult.Yes)
                    {
                        objdatlogisticaneg.ActuaArea(
                        textBox_CodArea.Text,
                        textBox_DesArea.Text,
                        comboBox_Gerencia.SelectedValue.ToString(),
                        textbox_ObsArea.Text,
                        comboBox_EstArea.SelectedValue.ToString(),
                        ...
                        Msbox.Frm_Notificaciones ng = new Msbox.Frm_Notificaciones("AREA ACTUALIZADO", Color.FromArgb(13, 93, 140), 1);
                        ng.ShowDialog();
                        this.Close();
                ...
                else if (textBox_CodArea.Text != ce_logistica.CodTiExist)
                {
                    string MessageBoxTitle = "Guardar";
                    string MessageBoxContent = "¿Desea Guardar Area?";

                    DialogResult dialogResult = MessageBox.Show(MessageBoxContent, MessageBoxTitle, MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
                    if (dialogResult == DialogResult.Yes)
                    {
                        objdatlogisticaneg.GuardArea(
                        textBox_CodArea.Text,
                        textBox_DesArea.Text,
                        comboBox_Gerencia.SelectedValue.ToString(),
                        textbox_ObsArea.Text,
                        ...
                        Msbox.Frm_Notificaciones ng = new Msbox.Frm_Notificaciones("AREA AGREGADO", Color.FromArgb(13, 93, 140), 1);
                        ng.ShowDialog();
                        this.Close();
                    }
                }
        ...
        private void EliminarArea()
        {
            ...
            if (textBox_CodArea.Text == ce_logistica.CodTiExist)
            {
                ...
                string MessageBoxContent = "¿Desea eliminar Area?";

                DialogResult dialogResult = MessageBox.Show(MessageBoxContent, MessageBoxTitle, MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
                if (dialogResult == DialogResult.Yes)
                {
                    objdatlogisticaneg.EliminArea(textBox_CodArea.Text);
                    Msbox.Frm_Notificaciones ng = new Msbox.Frm_Notificaciones("AREA ELIMINADO", Color.OrangeRed, 3);
                    ng.ShowDialog();
                    Close();
                    CargarPanelgeneral();
                    Limpiartextbox();
            ...
            {
                Msbox.Frm_Notificaciones ng = new Msbox.Frm_Notificaciones("AREA NO ENCONTRADO", Color.OrangeRed, 3);
                ng.ShowDialog();
            }
        }
