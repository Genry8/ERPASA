...
namespace ERPASA.Frm_Sistema
{
    ...
    public partial class frm_BuscarPersonal : Form
    {
        ...
        public frm_BuscarPersonal()
        {
            InitializeComponent();
            //FormBorderStyle = FormBorderStyle.None;            
        }

        private void frm_BuscarPersonal_Load(object sender, EventArgs e)
        {
            comboBox_TiposPer.SelectedIndex = 0;
            comboBox_TiposPer.DropDownStyle = ComboBoxStyle.DropDownList;
            textbox_PersonalDesc.Focus();

            textbox_PersonalDesc.CharacterCasing = CharacterCasing.Upper;
            dataGridView2.Columns.Clear();
            dataGridView2.DataSource = cn_logistica.ListaPersonalAsigTI();
        ...
        private void dataGridView2_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            if (dataGridView2.CurrentRow.Cells[8].Value.ToString() == "ANULADO")
            {
                Msbox.Frm_Notificaciones ng = new Msbox.Frm_Notificaciones("USUARIO ELIMINADO", Color.Red, 3);
                ng.ShowDialog();
                //break;
            }
        ...
        private void textbox_PersonalDesc_TextChanged(object sender, EventArgs e)
        {

        }

        private void comboBox_TiposPer_Click(object sender, EventArgs e)
        {
            dataGridView2.ClearSelection();
        }

        private void textbox_PersonalDesc_KeyPress(object sender, KeyPressEventArgs e)
        {
            if ((int)e.KeyChar == (int)Keys.Enter)
            {
                if (comboBox_TiposPer.Text == "DNI")
                {
                    dataGridView2.Columns.Clear();
                    dataGridView2.DataSource = cn_logistica.ListaPersonalAsigDniTI(textbox_PersonalDesc.Text);
                    //dataGridView2.DataSource = cn_nisira.ListaPersonalNisiraDni(textbox_PersonalDesc.Text);
                    dataGridView2.AllowUserToAddRows = false;
                    OrigenColorfila();
                    dataGridView2.ClearSelection();
                }
                if (comboBox_TiposPer.Text == "APELLIDO")
                {
                    dataGridView2.Columns.Clear();
                    dataGridView2.DataSource = cn_logistica.ListaPersonalAsigApeTI(textbox_PersonalDesc.Text);
                    //dataGridView2.DataSource = cn_nisira.ListaPersonalNisiraApellido(textbox_PersonalDesc.Text);
                    dataGridView2.AllowUserToAddRows = false;
                    OrigenColorfila();
                    dataGridView2.ClearSelection();
                }

                if (comboBox_TiposPer.Text == "NOMBRE")
                {
                    dataGridView2.Columns.Clear();
                    dataGridView2.DataSource = cn_logistica.ListaPersonalAsigNomTI(textbox_PersonalDesc.Text);
                    //dataGridView2.DataSource = cn_nisira.ListaPersonalNisiraNombre(textbox_PersonalDesc.Text);
                    dataGridView2.AllowUserToAddRows = false;
                    OrigenColorfila();
                    dataGridView2.ClearSelection();
                }

                if (comboBox_TiposPer.Text == "AREA")
                {
                    dataGridView2.Columns.Clear();
                    dataGridView2.DataSource = cn_logistica.ListaPersonalAsigAreaTI(textbox_PersonalDesc.Text);
                    dataGridView2.AllowUserToAddRows = false;
                    OrigenColorfila();
                    dataGridView2.ClearSelection();
