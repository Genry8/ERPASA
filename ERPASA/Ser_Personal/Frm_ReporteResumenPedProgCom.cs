...
{
    public partial class Frm_ReporteResumenPedProgCom : Form
    {
        ...
        public Frm_ReporteResumenPedProgCom()
        {
            InitializeComponent();
        ...
        private void Frm_ReporteResumenPedProgCom_Load(object sender, EventArgs e)
        {
        ...
        private void toolStripButton_buscar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                BuscarUsuarioPedateadoInasistencia();
        ...
        public void ImprimirReporteComedor()
        {
            ...
            for (int i = 0; i <= filas - 1; i++)
            {
                objDsRepVenta.Tables[3].Rows.Add
                    (new object[]{
                    dataGridView2[0,i].Value.ToString(),//NRO
                    dataGridView2[1,i].Value.ToString(),//DNI, DESPUES DE DNI SIGUE FECHA
                    //dataGridView1[2,i].Value.ToString(),//FECHA
                    Convert.ToString( Convert.ToDateTime(dataGridView2[2,i].Value.ToString()).Year + "-" + Convert.ToString(Convert.ToDateTime(dataGridView2[2,i].Value.ToString()).Month.ToString("D2")) + "-" + Convert.ToString(Convert.ToDateTime(dataGridView2[2,i].Value.ToString()).Day.ToString("D2")) ),
                    dataGridView2[3,i].Value.ToString(),//APE NOM
                    dataGridView2[4,i].Value.ToString(),//AREA
                    dataGridView2[5,i].Value.ToString(),//EMPRESA
                    dataGridView2[6,i].Value.ToString(),//TIP PERSONAL
                    dataGridView2[7,i].Value.ToString(),//ADICIONAL
                    dataGridView2[8,i].Value.ToString(),//DESAYUNO
                    dataGridView2[9,i].Value.ToString(),//ALM
                    dataGridView2[10,i].Value.ToString(),//CENA
        ...
        public void BuscarUsuarioPedateadoInasistencia()
        {
            String dni, adicional, area, tipo, tipopersonal, emp, sede;
            ...
            dataGridView1.DataSource = cn_logistica.BuscarPedidoProgramadoComedorResumen(
            dateTimePicker_Comedor_F_Inicio.Value.Date, dateTimePicker_Fecha_Final.Value.Date
            , tipopersonal, emp, sede);
            ...
            dataGridView2.DataSource = cn_logistica.BuscarPedidoRegistro(
            dateTimePicker_Comedor_F_Inicio.Value.Date, dateTimePicker_Fecha_Final.Value.Date
            , emp);
            dataGridView2.AllowUserToAddRows = false;
            //SumarTotal();
            ...
            {
                label_cantreg.Text = (dataGridView2.RowCount).ToString() + " REGISTROS";
        ...
        public void OrigenColorfila()
        {
            ...
            for (int i = 0; i < c; i++)
            {
                if (dataGridView2.Rows[i].Cells[3].Value.ToString() == "ANULADO")
                {
        ...
        public void SumarTotal()
        {
            ...
            Decimal solesdesayuno = 0;
            Decimal solesalmuerzo = 0;
            Decimal solescena = 0;
            Decimal total = 0;
            //decimal otros = 0;
            foreach (DataGridViewRow item in dataGridView2.Rows)
            {
                ...
                //for (int i = 0; i < c; i++)
                //{
                if (item.Cells["DESAYUNO"].Value.ToString().Contains("NO"))
                {
                    desayuno = desayuno + 1;
                }
                if (item.Cells["ALMUERZO"].Value.ToString().Contains("NO"))
                {
                    almuerzo = almuerzo + 1;
                }
                if (item.Cells["CENA"].Value.ToString().Contains("NO"))
                {
                    cena = cena + 1;
                }
                //int suma = 0;               
            ...
            //label_cantidad_total.Text = (desayuno + almuerzo + cena).ToString();
        ...
        private void Imprimir_grid_1_Click(object sender, EventArgs e)
        {
        ...
        private void Imprimir_grid_2_Click(object sender, EventArgs e)
        {
            dataGridView2.AllowUserToAddRows = false;
            progressBar1.Visible = true;
            label_exportar_excel.Visible = true;
            ...
            dataGridView2.AllowUserToAddRows = false;
            objexcel_progress.exportarexcelTimer(dataGridView2, progressBar1);
            progressBar1.Visible = false;
            label_exportar_excel.Visible = false;
        }
