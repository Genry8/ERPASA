...
{
    public partial class Frm_DerechoHambiente_Registrar : Form
    {
        ...
        public Frm_DerechoHambiente_Registrar()
        {
            InitializeComponent();
        ...
        private void Frm_DerechoHambiente_Registrar_Load(object sender, EventArgs e)
        {

            //ALTO RIESGO
            comboBox_tipo.SelectedIndex = 0;
            comboBox_CodSede_Comedor.SelectedIndex = 0;
        ...
        private void comboBox_empresa_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            //{
            //    //CARGAR SEDE EMPRESA
            //    comboBox_CodSede.DataSource = cn_logistica.ListaCBSedeEmpresaAsignacionTI(comboBox_empresa.Text);
        ...
        private void textBox_Dni_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            if ((int)e.KeyChar == (int)Keys.Enter)
            {
                ...
                //cn_logistica.BuscExistePersonalComedor(textBox_Dni.Text.Trim(), comboBox_CodSede_Comedor.Text, currentDateTime);
                ...
                if (comboBox_tipo.Text != "TODOS")
                {
                    if (comboBox_CodSede_Comedor.Text != "COMEDOR")
                    {
                        ...
                        {
                            if (textBox_Dni.Text == ce_logistica.CodUsuarioExist)
                            {
                                //textBox_ApeNom.Text = ce_logistica.NombresUsuario;
                                label_nombre_usuario.Text = ce_logistica.NombresUsuario;
                                if (comboBox_CodSede_Comedor.Text == ce_logistica.UsuarioSede)
                                {
                                    GuardarUsuarioPedateado();
                                }
                                else
                                {
                                    string tipo = "EL USUARIO PERTENECE A " + ce_logistica.UsuarioSede;
        ...
        public void GuardarUsuarioPedateado()
        {
            ...
            cn_logistica.BuscExistePedateoPersonalComedor(textBox_Dni.Text, currentDateTime, comboBox_tipo.Text);
            if (textBox_Dni.Text == ce_logistica.CodPedateoExist & comboBox_tipo.Text == ce_logistica.CodPedateoTiExist)
            {
                Msbox.Frm_NotificacionesPeligro ng = new Msbox.Frm_NotificacionesPeligro("EL USUARIO YA MARCO: " + ce_logistica.CodPedateoTiExist, Color.FromArgb(238, 24, 24), 3);
                ng.ShowDialog();
                textBox_Dni.Text = "";
            ...
            {
                cn_logistica.BuscExistePedateoPersonalComedorAlimento(textBox_Dni.Text, currentDateTime, comboBox_tipo.Text);
                if (textBox_Dni.Text == ce_logistica.CodPedateoExistAlimento & comboBox_tipo.Text == ce_logistica.CodPedateoTipoAlimentoExist)
                {
                    objcnlog.GuardarPedateoPersonalComedor(
                                ...
                                ce_logistica.NombresUsuario,//APELLIDOS Y NOMBRES
                    ...
                    Msbox.Frm_NotificacionesGuardar ng = new Msbox.Frm_NotificacionesGuardar("MARCADO CORRECTAMENTE ", Color.FromArgb(0, 179, 78), 2);
                    ng.Show();
        ...
        public void BuscarUsuarioPedateado()
        {
            ...
            dataGridView1.DataSource = cn_logistica.BuscarPedateoPersonalComedor(
                textBox_Dni.Text, "%", currentDateTime, comboBox_CodSede_Comedor.Text);
            dataGridView1.AllowUserToAddRows = false;
            SumarTotal();
            OrigenColorfila();
            dataGridView1.ClearSelection();
