...
{
    public partial class Frm_CargarRiegoProgramaPulsos : Form
    {
        ...
        public Frm_CargarRiegoProgramaPulsos()
        {
            InitializeComponent();
        }

        private void Frm_CargarRiegoPH_CE_Load(object sender, EventArgs e)
        {
            //ALTO RIESGO
            //poner osucro

            comboBox_cabezal.SelectedIndex = 0;
            //comboBox_TipoAguaSuelo.SelectedIndex = 0;
            ...
            //comboBox_TipoAguaSuelo.DropDownStyle = ComboBoxStyle.DropDownList;
        ...
        public void GuardarPH_CE()
        {
            ...
            if (dialogResult == DialogResult.Yes)
            {
                ...
                {
                    var exrev = cn_administracion.BuscExistePulsos(
                        //comboBox_empresa.Text,
                        comboBox_Sede.Text,
                        comboBox_cabezal.Text,
                        dataGridView1[0, i].Value.ToString().ToUpper().Trim(),//variedad
                        Convert.ToInt32(dataGridView1[1, i].Value),//valvula
                        Convert.ToDateTime(dataGridView1[2, i].Value).Date //Fecha
                        );

                    if (exrev == true)
                    {
                        objcnlog.ActualizarPulsos(
                        //comboBox_empresa.Text,
                        comboBox_Sede.Text,
                        comboBox_cabezal.Text,
                        dataGridView1[0, i].Value.ToString().ToUpper().Trim(),//variedad
                        Convert.ToInt32(dataGridView1[1, i].Value),//valvula
                        Convert.ToDateTime(dataGridView1[2, i].Value).Date, //Fecha
                        Convert.ToDecimal(dataGridView1[3, i].Value),// pulsos
                        dataGridView1[4, i].Value.ToString().ToUpper().Trim()//obs
        ...
        private void comboBox_cabezal_SelectedIndexChanged(object sender, EventArgs e)
        {
        ...
        private void comboBox_TipoAguaSuelo_SelectedIndexChanged(object sender, EventArgs e)
        {
