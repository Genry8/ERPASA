...
{
    public partial class frm_EditarGrupoVehiculo : Form
    {
        private DataSet dtsTablas = new DataSet();
        cn_taller objcntaller = new cn_taller();
        cn_planilla objcnlog = new cn_planilla();

        public frm_EditarGrupoVehiculo()
        {
            InitializeComponent();
        }

        private void frm_EditarGrupoVehiculo_Load(object sender, EventArgs e)
        {
            //AgregarColumna();
        }
        ...
        public void PonerOscuroComboBox()
        {
            ...
            textBox_Grupo.CharacterCasing = CharacterCasing.Upper;
        ...
        {
            if (textBox_Grupo.Text.Trim() == "")
            {
                Msbox.Frm_NotificacionesPeligro ng =
                    new Msbox.Frm_NotificacionesPeligro("FALTA DESCRIPCION DE GRUPO", Color.FromArgb(238, 24, 24), 3);
                ng.ShowDialog();
            }
            else
            {
                ...
                string MessageBoxContent = "¿Desea actualizar grupo? ";

                DialogResult dialogResult = MessageBox.Show(MessageBoxContent, MessageBoxTitle, MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
                if (dialogResult == DialogResult.Yes)
                {

                    objcntaller.ActualizarGrupoVehiculo(
                        label_Id.Text,
                    textBox_Grupo.Text.Trim(),//grupo
