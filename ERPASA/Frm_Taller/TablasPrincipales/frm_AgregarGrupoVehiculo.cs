...
{
    public partial class frm_AgregarGrupoVehiculo : Form
    {
        private DataSet dtsTablas = new DataSet();
        cn_taller objcntaller = new cn_taller();
        cn_planilla objcnlog = new cn_planilla();

        public frm_AgregarGrupoVehiculo()
        {
            InitializeComponent();
        }

        private void frm_AgregarGrupoVehiculo_Load(object sender, EventArgs e)
        {
            PonerOscuroComboBox();
            CargarDatosComboBox();
            //AgregarColumna();
        ...
        public void AlertaGuardarVehiculos()
        {
            ...
            else
            {
                ...
                //if (exchar == false)
                {
                    ...
                    string MessageBoxContent = "¿Desea guardar grupo? ";

                    DialogResult dialogResult = MessageBox.Show(MessageBoxContent, MessageBoxTitle, MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
                    if (dialogResult == DialogResult.Yes)
                    {

                        objcntaller.GuardarGrupoVehiculo(
